...
class SlideViewLayout: UICollectionViewLayout {
    
    internal var contentSize: CGSize = .zero
    internal var leadingSpacing: CGFloat = 0
    internal var itemSpacing: CGFloat = 0
    internal var needsReprepare = true
    internal var scrollDirection: SlideView.ScrollDirection = .horizontal
    
    open override class var layoutAttributesClass: AnyClass {
        return SlideViewLayoutAttributes.self
    }
    
    fileprivate var pagerView: SlideView? {
        return self.collectionView?.superview?.superview as? SlideView
    }
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        while origin-maxPosition <= max(CGFloat(100.0) * .ulpOfOne * abs(origin+maxPosition), .leastNonzeroMagnitude) {
            ...
            let attributes = self.layoutAttributesForItem(at: indexPath) as! SlideViewLayoutAttributes
    ...
    override open func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        let attributes = SlideViewLayoutAttributes(forCellWith: indexPath)
    ...
    override open func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint, withScrollingVelocity velocity: CGPoint) -> CGPoint {
        ...
        func calculateTargetOffset(by proposedOffset: CGFloat, boundedOffset: CGFloat) -> CGFloat {
            ...
            if pagerView.decelerationDistance == SlideView.automaticDistance {
    ...
    fileprivate func applyTransform(to attributes: SlideViewLayoutAttributes, with transformer: SlideViewTransformer?) {
        guard let collectionView = self.collectionView else {
            return
        }
