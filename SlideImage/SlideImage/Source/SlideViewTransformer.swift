...
@objc
public enum SlideViewTransformerType: Int {
    case crossFading
    case zoomOut
    case depth
    case overlap
...
open class SlideViewTransformer: NSObject {
    
    open internal(set) weak var pagerView: SlideView?
    open internal(set) var type: SlideViewTransformerType
    
    open var minimumScale: CGFloat = 0.65
    open var minimumAlpha: CGFloat = 0.6
    
    @objc
    public init(type: SlideViewTransformerType) {
        self.type = type
        switch type {
        case .zoomOut:
    ...
    open func applyTransform(to attributes: SlideViewLayoutAttributes) {
