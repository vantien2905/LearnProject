...
open class RangeSlider: UIControl {
   ...
   var rangeValues = Array(10...20) {
    ...
    @IBInspectable open var thumbSize: CGFloat = 16.0 {
        didSet {
            thumbRadius = thumbSize / 2.0
            updateLayerFrames()
        }
    ...
    func updateLayerFrames() {
        ...
        let displayLayerOffsetY = offsetY - thumbRadius - 12
        ...
        minValueDisplayLayer.frame = CGRect(x: position(rangeValues.first ?? 0)-thumbRadius*2 ,//minValuePosition,
                                            y: displayLayerOffsetY,
                                            width: thumbSize*2,
                                            height: displayTextFontSize)
        if let minValueDisplayText = minValueDisplayTextGetter?(minValue) {
            if minValue == rangeValues.first ?? 0 {
        ...
        maxValueDisplayLayer.frame = CGRect(x: position(rangeValues.last ?? 0)-thumbRadius*2.5,
                                            y: displayLayerOffsetY,
                                            width: thumbSize*2,
                                            height: displayTextFontSize)
        if let maxValueDisplayText = maxValueDisplayTextGetter?(maxValue) {
            if maxValue == rangeValues.last ?? 0 {
