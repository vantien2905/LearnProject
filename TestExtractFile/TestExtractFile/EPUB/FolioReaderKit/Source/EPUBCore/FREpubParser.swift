...
class FREpubParser: NSObject, SSZipArchiveDelegate {
    ...
    func readEpub(epubPath withEpubPath: String, removeEpub: Bool = true, unzipPath: String? = nil) throws -> FRBook {
        ...
        let bookName = String(withEpubPath.lastPathComponent)
        var bookBasePath = ""
        ...
        let epubURL = URL(string: withEpubPath)!
        let destinationURL = URL(string: bookBasePath)!

        self.epubExtractor.extractEpub(epubURL: epubURL, destinationFolder: destinationURL)
