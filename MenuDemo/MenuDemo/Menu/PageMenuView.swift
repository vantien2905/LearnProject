...
//        view.setBorder(cornerRadius: 10)
        return view
    }()
    ...
    var heightHorizontal: CGFloat = 6.5
    var spaceBottom: CGFloat = 0
    ...
    func setUpViewMenu(menuColorBackground: UIColor, menuFont: UIFont, menuColorNormal: UIColor, menuColorSelected: UIColor, menuColorHorizontal: UIColor, heightHorizontal: CGFloat, listItem: [Category], isFull: Bool = false, isHaveLineTop: Bool = false) {
...
extension PageMenuView: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    func configureCollection() {
        cvMenu.register(PageMenuViewCell.self, forCellWithReuseIdentifier: cellId)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! PageMenuViewCell
...
//        let wid = collectionView.frame.width /  CGFloat(listItem.count)
        let itemStr = listItem[indexPath.item]
        var widthAll: CGFloat = 0
        for item in listItem {
            guard let itemStr = item as? Category else {return 0}
        ...
        guard let _itemCurrent = listItem[indexPath.item] as? Category else {return 0}
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if isFullView {
            ...
            if indexPath.item == 0 && vHorizotal.frame.width == 0 {
                vHorizotal.frame = CGRect(x: 0, y: cvMenu.frame.height - heightHorizontal - spaceBottom , width: wid, height: heightHorizontal)
            }
            
            return CGSize(width: wid, height: collectionView.frame.height)
        } else {
            if let itemStr = listItem[indexPath.item] as? Category {
    ...
    func setHorizontal(index: Int, isFullCell: Bool = true) {
        ...
        if !isFullCell {
            self.vHorizotal.frame = CGRect(x: x, y: cvMenu.frame.height - heightHorizontal - spaceBottom, width: widthCell, height: heightHorizontal)
        } else {
            self.vHorizotal.frame = CGRect(x: x, y: cvMenu.frame.height - heightHorizontal - spaceBottom, width: widthCell, height: heightHorizontal)
        }
    ...
    func scrollToIndex(index: Int) {
        ...
        guard let cellScroll = cvMenu.cellForItem(at: indexPath) as? PageMenuViewCell else { return }
        
        UIView.animate(withDuration: 0.3) {
            //-- scroll view horizontal
            self.vHorizotal.frame = CGRect(x: self.vHorizotal.frame.minX, y: self.cvMenu.frame.height - self.heightHorizontal - self.spaceBottom, width: cellScroll.frame.width, height: self.heightHorizontal)
            ...
            for indexPath in listIndexPathVisible {
                if let cell = self.cvMenu.cellForItem(at: indexPath) as? PageMenuViewCell {
            ...
            //--
            guard let listCategory = self.listItem as? [Category] else { return }
...
class PageMenuViewCell: UICollectionViewCell {
    let lbTitle: UILabel = {
        let lb = UILabel()
...
//        img.image = UIImage(named: "lock")
    ...
    func setData(item: AnyObject?, font: UIFont, normalColor: UIColor, selectedColor: UIColor) {
        if let cate = item as? Category {
            lbTitle.text = cate.title
            lbTitle.font = font
            lbTitle.textColor = cate.isSelected == true ? selectedColor : normalColor
            imgIcon.tintColor = cate.isSelected == true ? selectedColor : normalColor
            imgIcon.image = cate.image
        }
    }
    ...
    func setUpView() {
        ...
        imgIcon.anchor(self.topAnchor, topConstant: 0, widthConstant: 20, heightConstant: 20)
        imgIcon.centerXToSuperview()
        
        lbTitle.centerXToSuperview()
        lbTitle.anchor(imgIcon.bottomAnchor, topConstant: 5)
    }
}
...
class Category {
    ...
    init(title: String, isSelected: Bool, image: UIImage) {
        self.title = title
        self.image = image
